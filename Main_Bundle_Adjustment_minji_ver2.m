...
for i = 2:tracking_num
    ...
    T_relative = inv(T_p2c_cam{1}) * T_p2c_cam{i} ;
    ...
    objective_function = @(params) EReprojection_motiononly3(params, InitialMap_PM, InitialMap_Win, C_pm{i}, C_win{i}, intrinsics.K);
    ...
    [opt_params, resnorm] = lsqnonlin(objective_function, initial_params, [], [], options);
    ...
    T_relative = inv( inv(T_p2c_cam{1}) * T_p2c_cam{i} );
    Origin_R=T_relative(1:3,1:3);
    Origin_t=T_relative(1:3,4);
    ...
    for j = 1:sample_num
        ...
        %visualize reprojection with optimized R,t
    for j = 1:sample_num
        P_camera = Origin_R * InitialMap_PM(:, j) + Origin_t;
        % Convert to homogeneous coordinates by appending a row of ones
        P_camera_homogeneous = P_camera;
        ...
        % Extract the 2D points
        u1_orgin(j) = projected_points(1, :);
        v1_origin(j) = projected_points(2, :);
