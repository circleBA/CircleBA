...
for i =1:tracking_num
    ...
    % 
    % tstampStr = sprintf('%.0f', tstamp(i));
    % image_path = strcat(imagePath, imgList(i, :));
    % I = imread(image_path);      % ✅ 이미지 로드
    % imshow(I); hold on;
    % 
    % 
    % draw_conic_Q_on_image(C_pm{i}/norm(C_pm{i}, 'fro'), 'b', size(I));   % PM ellipse
    % draw_conic_Q_on_image(C_win{i}/norm(C_win{i}, 'fro'), 'r', size(I)); % Win ellipse
    % 
    % % ==== Project 3D points ====
    % u1 = zeros(1, sample_num);
    % v1 = zeros(1, sample_num);
    % u2 = zeros(1, sample_num);
    % v2 = zeros(1, sample_num);
    % for j = 1:sample_num
    %     % Use T_relative, not optimized one
    %     P_pm = T_relative(1:3,1:3) * InitialMap_PM(:, j) + T_relative(1:3,4);
    %     P_win = T_relative(1:3,1:3) * InitialMap_Win(:, j) + T_relative(1:3,4);
    % 
    %     proj_pm = intrinsics.K * P_pm;
    %     proj_win = intrinsics.K * P_win;
    %     proj_pm = proj_pm / proj_pm(3);
    %     proj_win = proj_win / proj_win(3);
    % 
    %     u1(j) = proj_pm(1); v1(j) = proj_pm(2);
    %     u2(j) = proj_win(1); v2(j) = proj_win(2);
    % end
    % 
    % 
    % plot(u1, v1, 'mo', 'DisplayName', 'PM Reproj', 'MarkerSize', 2, 'LineWidth', 0.5);
    % plot(u2, v2, 'co', 'DisplayName', 'Win Reproj', 'MarkerSize', 2, 'LineWidth', 0.5);
    % 
    % 
    % title(sprintf('Frame %d: Conic Q vs Reprojection', i));
    % axis equal;
...
function residuals = EReprojection_motiononly3(params, PM_Point3D, Win_Point3D, Qpm, Qwin, K)
    ...
    for i = 1:N
        ...
        residuals(i) = sqrt(0*(val_pm)^2 +1000*(val_win)^2);
