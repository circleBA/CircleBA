...
T_w2c_iss = T_w2c_cam{1} * inv(T_iss_cam_true{1});
for i = 1:tracking_num
...
for i = 2:tracking_num
    %T_relative = inv(T_p2c_cam{1}) * T_p2c_cam{i} ; % 원래 초기값은 P2C
    % T_relative = inv( inv(T_p2c_cam{1}) * T_p2c_cam{i} ); % 원래 초기값은 P2C
    
    T_relative = inv( inv(T_w2c_cam{1}) * T_w2c_cam{i} );
    % T_relative = eye(4);
    ...
    % J = computeJacobian(@(x) EReprojection_motiononly3( ...
    % x, ...
    % InitialMap_PM, ...
    % InitialMap_Win, ...
    % C_pm{i}, ...
    ...
    EstimatedPose{i} = T_w2c_cam{1} * inv( Relative_Trans_opt );
    ...
    % figure; hold on; axis equal;
    % xlim([0, image_size(2)]); ylim([0, image_size(1)]);
    % set(gca, 'YDir', 'reverse');  % 이미지 좌표계와 맞추기
    % title('Synthetic Image with Reprojected Circles');
    % xlabel('X-axis (pixels)'); ylabel('Y-axis (pixels)');
    % 
    % k = ellipses_params{1,10}(1,1); % y
    % h = ellipses_params{1,10}(1,2); % x
    % a = ellipses_params{1,10}(1,3)/2;
    % b = ellipses_params{1,10}(1,4)/2;
    % angle = 90 - ellipses_params{1,10}(1,5);
    % 
    % theta = deg2rad(angle);
    ...
    for j = 1:sample_num
        ...
        % 모든 projected point 시각화
        % scatter(projected_points(1, :), projected_points(2, :), 10, 'r', 'filled');
    ...
    % hold off;
    % filename = sprintf('visualization/reprojection_result_%02d.png', i);  % 프레임 인덱스 붙이기
    % saveas(gcf, filename);
...
%% Functions
function ellipse_params = fit_ellipse_nonlinear(x, y)
    xy = [x(:), y(:)];
    x0 = mean(x); y0 = mean(y);
    a0 = (max(x)-min(x))/2; b0 = (max(y)-min(y))/2;
    theta0 = 0;

    init = [x0, y0, a0, b0, theta0];
    options = optimoptions('lsqnonlin','Display','off');
    result = lsqnonlin(@(p) ellipse_residual(p, xy), init, [], [], options);

    ellipse_params = [result(2), result(1), 2*result(3), 2*result(4), rad2deg(result(5))];
end
...
function ellipse_params = fit_ellipse_conic(x, y)
    % 입력: x, y는 열벡터 (N×1)
    ...
    valid = find(real(diag(eig_val)) > 0 & ~isinf(diag(eig_val)));
    a = real(eig_vec(:, valid(1)));

    A = a(1); B = a(2); C_ = a(3); D = a(4); E = a(5); F = a(6);
    delta = B^2 - 4*A*C_;
    x0 = (2*C_*D - B*E)/delta;
    y0 = (2*A*E - B*D)/delta;

    num = 2*(A*E^2 + C_*D^2 + F*B^2 - 2*B*D*E - A*C_*F);
    term = sqrt((A - C_)^2 + B^2);
    a_len = sqrt(num / (delta * ((A + C_) - term)));
    b_len = sqrt(num / (delta * ((A + C_) + term)));

    if b_len > a_len
        [a_len, b_len] = deal(b_len, a_len);
        theta = 0.5 * atan2(B, A - C_) + pi/2;
    else
        theta = 0.5 * atan2(B, A - C_);
    end

    ellipse_params = [y0, x0, 2*a_len, 2*b_len, rad2deg(theta)];
    ellipse_params = real(ellipse_params);
end
...
function residuals = EReprojection_motiononly2(params, PM_Point3D, Win_Point3D, Qpm, Qwin, K)
    % INPUTS:
