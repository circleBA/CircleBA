...
for i=1:tracking_num
    ...
    % plot_camera_frame( T_relative{i}(1:3, 1:3), T_relative{i}(1:3, 4), 0.5, 'm');
    plot_camera_frame( T_p2c_cam{best_k}(1:3, 1:3), T_p2c_cam{best_k}(1:3, 4), 0.5, 'k');
    % plot_camera_frame( T_cam_p2c{best_k}(1:3, 1:3), T_cam_p2c{best_k}(1:3, 4), 0.5, 'm');
...
for i = 40:40
    ...
    for j = 1:sample_num
        % Step 1: get T_best and T_i1
        T_best = T_p2c_cam_true{best_k};       % 월드 ← best_k
        T_next = T_p2c_cam_true{i};          % 월드 ← i+1

        % Step 2: 변환 (best_k → world → i+1)
        X_bk = InitialMap_Win(:, j);           % 3x1, best_k 기준
        X_bk_h = [X_bk; 1];                     % homogeneous

        X_world = inv(T_best) * X_bk_h;        % 월드 기준
        X_i1 = T_next * X_world;               % i+1 카메라 기준
        X_i1 = X_i1(1:3);                      % 3x1

        % Step 3: projection
        x_proj = intrinsics.K * X_i1;
        u2_true = x_proj(1) / x_proj(3);
        v2_true = x_proj(2) / x_proj(3);
        % T= inv(EstimatedPose{best_k}) * T_p2c_cam_true{i};
        % R = T(1:3, 1:3);
        % t = T(1:3, 4);
        %
        % P_camera = R *InitialMap_Win(:, j)+t ;
        % 
        % % Convert to homogeneous coordinates by appending a row of ones
        % P_camera_homogeneous = P_camera;
        % 
        ...
        % 
        % % Extract the 2D points
        % u2_true(j) = projected_points(1, :);
        % v2_true(j) = projected_points(2, :);
...
for i = 1:tracking_num
    ...
    % plot_camera_frame(T_p2c_cam_true{i}(1:3, 1:3), T_p2c_cam_true{i}(1:3, 4), 0.5, 'k'); hold on;
    % plot_camera_frame(EstimatedPose{best_k}(1:3, 1:3), EstimatedPose{best_k}(1:3, 4), 0.5, 'r');
    plot_camera_frame(T_p2c_cam{best_k}(1:3, 1:3), T_p2c_cam{best_k}(1:3, 4), 0.5, 'g');
    plot_camera_frame(T_p2c_cam{i}(1:3, 1:3), T_p2c_cam{i}(1:3, 4), 0.5, 'm');
    transformed_points = T_p2c_cam{i}(1:3, 1:3) * InitialMap_PM_all{i} + T_p2c_cam{i}(1:3, 4);
    transformed_points_win =T_p2c_cam{i}(1:3, 1:3) * InitialMap_Win_all{i} + T_p2c_cam{i}(1:3, 4);
    % plot3(transformed_points(1, :), transformed_points(2, :), transformed_points(3, :), 'c', 'MarkerSize', 0.1);
    % plot3(transformed_points_win(1, :), transformed_points_win(2, :), transformed_points_win(3, :), 'm', 'MarkerSize', 0.1);
    grid on; axis equal;
    xlabel('X'); ylabel('Y'); zlabel('Z');
