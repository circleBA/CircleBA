...
% R_relative_opt = cell(tracking_num, 1);
% T_relative_opt = cell(tracking_num, 1);
% T_relative = cell(tracking_num, 1);  % 또는 zeros(4,4,tracking_num) 써도 됨
% for i = 1:tracking_num
%     if i == best_k
%         continue;
%     end
%     T_relative{i} = inv( inv(T_w2c_cam{best_k}) * T_w2c_cam{i} );
%     R_relative_opt{i} = T_relative{i}(1:3, 1:3);  % 회전 행렬
%     T_relative_opt{i}= T_relative{i}(1:3, 4);    % 위치 벡터
% 
%     tstampStr = sprintf('%.0f', tstamp(i));
%     image = strcat(imagePath, imgList(i, :));
%     for j = 1:sample_num
...
%     end
% 
%     plot_camera_frame(EstimatedPose{best_k}(1:3, 1:3), EstimatedPose{best_k}(1:3, 4), 1, 'g');
%     plot_camera_frame(T_p2c_cam_true{best_k}(1:3, 1:3), T_p2c_cam_true{best_k}(1:3, 4), 0.5, 'g');
%     plot_camera_frame(T_relative{i}(1:3, 1:3), T_relative{i}(1:3, 4), 0.5, 'r'); hold on;     % Relative_Trans_opt = expSE3(opt_params_store{i});  % opt_params를 미리 저장해둔 경우
...
for i =1:tracking_num
    ...
    T_relative = inv( inv(T_w2c_cam{best_k}) * T_w2c_cam{i} );
    ...
    R_relative_opt{i} = Relative_Trans_opt(1:3, 1:3);
    T_relative_opt{i} = Relative_Trans_opt(1:3, 4);
    
    % EstimatedPose{i} = T_p2c_cam{best_k} * inv(Relative_Trans_opt);
    EstimatedPose{i} = T_w2c_cam{best_k} * inv(Relative_Trans_opt);
...
for i = 1:tracking_num
    ...
    % plot_camera_frame(T_w2c_cam{i}(1:3, 1:3), T_w2c_cam{i}(1:3, 4), 0.5, 'c');
    ...
    plot_camera_frame(T_p2c_cam_true{best_k}(1:3, 1:3), T_p2c_cam_true{best_k}(1:3, 4), 0.5, 'b');
...
function residuals = EReprojection_motiononly3(params, PM_Point3D, Win_Point3D, Qpm, Qwin, K)
    ...
    % J = zeros(N, 6);
    % for j = 1:6
    %     d_params = zeros(6,1); d_params(j) = delta;
    %     T_perturbed = expSE3(params + d_params);
    %     res_perturbed = zeros(N,1);
    %     for i = 1:N
    %         % Perturbed projection
    %         pm_point3D_h = [PM_Point3D(:, i); 1];
    %         pm_P2_i_h = T_perturbed * pm_point3D_h;  % 4x1
    %         pm_P2_i = pm_P2_i_h(1:3);                % 3x1
    % 
    %         % Project and normalize
    %         pm_x2_h = K * pm_P2_i;
    %         if abs(pm_x2_h(3)) < 1e-8
    %             res_perturbed(i) = residuals(i);  % fallback to original residual
    %             continue;
    %         end
    %         pm_x2_h = pm_x2_h / pm_x2_h(3);
    % 
    %         % Residual under perturbation
    %         val_pm = pm_x2_h' * Qpm * pm_x2_h;
    %         res_perturbed(i) = sqrt(1000 * (val_pm)^2);  % same as above
    %     end
    %     J(:,j) = (res_perturbed - residuals) / delta;
    % end
    % 
    % % Hessian 계산
    % H = J' * J;
    % 
    % % 고유값 및 condition number 출력
    % eigenvals = eig(H);
    % condH = cond(H);
    % rankH = rank(H);
    % 
    % 
    % 
    % % fprintf("▶️ Eigenvalues of Hessian (J^T J):\n");
    % % disp(eigenvals);
    % % fprintf("▶️ Condition number of Hessian: %.4e\n", condH);
    % % 저장할 파일 경로 (원하는 경로로 수정하세요)
    % 
    % log_file = 'cond_rank_log.txt';
    % 
    % % 포맷팅된 로그 생성
    % log_entry = sprintf('%.4e %d\n', condH, rankH);
    % 
    % % 파일에 append
    % fid = fopen(log_file, 'a');   % 'a' = append 모드
    % fprintf(fid, '%s', log_entry);
    % fclose(fid);
    % 
    % 
    % % log_file = 'condition_rank_log.m';
    % % 
    % % % 분석 결과를 문자열로 포맷팅
    % % % log_entry = sprintf('[%s];\n', num2str(condH', '%.4e '));
    % % log_entry = sprintf('%.4e %d\n', condH, rankH);
    % % 
    % % 
    % % % 로그 파일에 append 방식으로 저장
    % % fid = fopen(log_file, 'a');  % 'a' = append
    % % fprintf(fid, '%s', log_entry);
    % % fclose(fid);
    % % save_path = 'conditiaon_log.mat';
    % % if isfile(save_path)
    % %     load(save_path, 'eigen_matrix');
    % %     eigen_matrix(end+1, :) = eigenvals';  % 다음 row에 저장
    % % else
    % %     eigen_matrix = eigenvals';  % 첫 프레임일 경우
    % % end
    % % save(save_path, 'eigen_matrix');
