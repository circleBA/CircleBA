...
timestamp = timestamp(ransac);
R_iss_cam = R_iss_cam(ransac);
t_iss_cam = t_iss_cam(ransac);
T_iss_cam = T_iss_cam(ransac);
T_p2c_cam = T_p2c_cam(ransac);
T_cam_world = T_cam_world(ransac);
...
% [RPE_RMSE_TVIO,RPE_TVIO] = calcRPE(stateEsti_TVIO, stateTrue, (1/cdt), 'RMSE');
% [ATE_RMSE_TVIO,ATE_TVIO] = calcATE(stateEsti_TVIO, stateTrue, 'RMSE');
