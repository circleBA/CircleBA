...
for i = 1:tracking_num
    ...
    xi = logSE3(T_relative);
    pose_params_init = [pose_params_init; xi];
    pose_index_map(end+1) = i;
end

options = optimoptions('lsqnonlin', 'Algorithm', 'levenberg-marquardt', ...
    'Display','iter','FiniteDifferenceType','forward');

[opt_params, ~] = lsqnonlin(@(params) BA_motiononly_cost(params, ...
    InitialMap_PM, InitialMap_Win, C_pm, C_win, intrinsics.K, best_k, pose_index_map), ...
    pose_params_init, [], [], options);

EstimatedPose = cell(1, tracking_num);
EstimatedPose{best_k} = eye(4);  % 기준 pose

for cnt = 1:length(pose_index_map)
    i = pose_index_map(cnt);
    xi = opt_params((cnt-1)*6+1 : cnt*6);
    T_rel = expSE3(xi);
...
function residuals = BA_motiononly_cost(params, InitialMap_PM, InitialMap_Win, C_pm_all, C_win_all, K, best_k, pose_index_map)
    tracking_num = length(C_pm_all);
    residuals = [];

    % 기준 pose는 identity (고정)
    T_best = eye(4);

    for cnt = 1:length(pose_index_map)
        i = pose_index_map(cnt);  % 최적화 대상 frame index

        % 현재 frame의 xi → T_rel
        xi = params((cnt-1)*6+1 : cnt*6);
        T_rel = expSE3(xi);  % best_k 기준 상대 pose
        T_i = T_best * inv(T_rel);  % 절대 pose로 변환

        % residual 계산: EReprojection_motiononly3_from_pose을 따로 정의
        res_i = EReprojection_motiononly3_from_pose(T_i, InitialMap_PM, InitialMap_Win, ...
                                                    C_pm_all{i}, C_win_all{i}, K);
        residuals = [residuals; res_i];
    end
end

function residuals = EReprojection_motiononly3_from_pose(T, PM_Point3D, Win_Point3D, Qpm, Qwin, K)
    N = size(PM_Point3D, 2);
    residuals = zeros(N,1);

    Qpm = Qpm / norm(Qpm, 'fro');
    ...
    for i = 1:N
        ...
        pm_P2 = T * [pm_point3D; 1];
        win_P2 = T * [win_point3D; 1];

        pm_x2_h = K * pm_P2(1:3);
        win_x2_h = K * win_P2(1:3);
