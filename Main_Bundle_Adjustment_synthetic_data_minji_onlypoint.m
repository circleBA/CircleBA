...
% load('EstimatedPose_after_BA.mat');
% EstimatedPose=EstimatedPose1';
for i = 1:tracking_num
    if i == 1
...
%save('EstimatedPose.mat', 'EstimatedPose');
...
function ellipse_params = fit_ellipse(points_2D)
    ...
    major_axis_length = 2 * sqrt(max_eigenvalue)*1.4;
    minor_axis_length = 2 * sqrt(min_eigenvalue)*1.4;
...
function residuals = EReprojection_motiononly3(params, PM_Point3D, Win_Point3D, Qpm, Qwin, ellipses_params,PM_center_cam , K)
    ...
    residuals(1) = (sqrt(mean((pm_center_proj(1:2)-reference_center(1:2)).^2)));
    disp(residuals)
