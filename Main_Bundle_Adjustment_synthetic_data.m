...
for i = 1:size(true_pose,1)
    ...
    imgName = sprintf('%19d.png', tstamp{i});
...
% rows = image_size(1);  % e.g., 880
% cols = image_size(2);  % e.g., 1280
% 
% for i = 1:tracking_num
% 
%     % -- 1) Create a blank white image of the desired size
%     synthetic_image = 255 * ones(rows, cols, 3, 'uint8');  % 3-channel RGB
% 
%     % circle1_points_2D{i} is Nx2 => each row is [x, y]
%     circle1_xy = circle1_points_2D{i};
%     circle2_xy = circle2_points_2D{i};
% 
%     % If there are no valid points (rare edge case), skip
%     if isempty(circle1_xy) || isempty(circle2_xy)
%         continue;
%     end
% 
%     % -- 2) Insert red markers for circle1
%     synthetic_image = insertMarker( ...
%         synthetic_image, ...
%         circle1_xy, ...              % Nx2 positions
%         'o', ...                     % marker type
%         'Color', 'black', ...
%         'Size', 1 ...                % you can adjust marker size
%     );
% 
%     % -- 3) Insert blue markers for circle2
%     synthetic_image = insertMarker( ...
%         synthetic_image, ...
%         circle2_xy, ...
%         'o', ...
...
%     );
% 
%     % -- 4) Save the image (no figure needed)
%     out_filename = sprintf('./data/synthetic/%19d.png', tstamp{i});
%     imwrite(synthetic_image, out_filename);
% end
% disp('Done generating and saving synthetic images.');
...
for i = 1:tracking_num
    ...
    if angle_diff{i} > 20
        ...
        if i == 1 
            ...
            figure_detected_two_ellipse_syn(image, ellipses_params{i}, tstampStr);
            pause(1);
        end
        tstampStr = sprintf('%.0f', tstamp{i});
        image = strcat(imagePath, imgList(i, :));
...
for i = 2:tracking_num
    ...
    objective_function = @(params) EReprojection_motiononly2( ...
    params, ...
    InitialMap_PM, ...
    InitialMap_Win, ...
    C_pm{i}, ...
...
function [A,B,C,D,E,F] = fitEllipseConic(x, y)
    % x,y: vectors of boundary points
    % We assume we have at least 5 points, typically many.
    Dmat = [x.^2, x.*y, y.^2, x, y, ones(size(x))];

    % Solve in the least-squares sense for p = [A; B; C; D; E; F]
    % Usually you might add a constraint, e.g. B^2 - 4AC < 0 for ellipse.
    % A quick approach: we can do an SVD and pick the last column:
    [~,~,V] = svd(Dmat,0);
    p = V(:,end);
