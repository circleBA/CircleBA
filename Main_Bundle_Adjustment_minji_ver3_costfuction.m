...
function residuals = EReprojection_motiononly3(params, PM_Point3D, Win_Point3D,ellipses_params, K)
    % INPUTS:
    ...
    for i = 1:N
        ...
        x_pm = pm_x2_h(1:2);  % [u; v]
        x_win = win_x2_h(1:2);




        kpm = ellipses_params(1, 1); %y
        hpm = ellipses_params(1, 2); %x
        apm = ellipses_params(1, 3)/2; %a
        bpm = ellipses_params(1, 4)/2; %b
        thetapm = deg2rad(90-ellipses_params(1, 5)); %angle (deg)
        Rpm = [cos(thetapm), -sin(thetapm);
            sin(thetapm),  cos(thetapm)];
        Lambdapm = diag([1/apm^2, 1/bpm^2]);
        dxpm = x_pm - [hpm; kpm];
        valpm = dxpm' * Rpm * Lambdapm * Rpm' * dxpm;

        kwin = ellipses_params(2, 1); %y
        hwin= ellipses_params(2, 2); %x
        awin = ellipses_params(2, 3)/2; %a
        bwin = ellipses_params(2, 4)/2; %b
        thetawin = deg2rad(90-ellipses_params(2, 5)); %angle (deg)
        Rwin = [cos(thetawin), -sin(thetawin);
            sin(thetawin),  cos(thetawin)];
        Lambdawin = diag([1/awin^2, 1/bwin^2]);
        dxwin = x_win - [hwin; kwin];
        valwin = dxwin' * Rwin * Lambdawin * Rwin' * dxwin;


        % residuals = sqrt( (valpm - 1)^2 + (valwin - 1)^2 );
        residuals = sqrt( huber(valpm - 1, 1.0)^2 + huber(valwin - 1, 1.0)^2 );
...
%%
function r = huber(e, delta)
    if abs(e) <= delta
        r = 0.5 * e^2;
    else
        r = delta * (abs(e) - 0.5 * delta);
    end
end
