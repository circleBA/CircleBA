...
% InitialMap_Win = [Window_circle_3D(1, :); Window_circle_3D(2, :); Window_circle_3D(3, :)];



%% Reproject test with true pose
% disp("true reprojection");
% figure;
% for i=2:tracking_num
%     T1_inv = inv(T_iss_cam{1});
%     T_relative = T1_inv * T_iss_cam{i};
...
for i = 2:tracking_num
    ...
    objective_function = @(params) EReprojection(params, InitialMap_PM, InitialMap_Win, C_pm{i}, C_win{i}, intrinsics.K);
...
plot3(InitialMap_PM(1, :), InitialMap_PM(2, :), InitialMap_PM(3, :), 'r');
plot3(InitialMap_Win(1, :), InitialMap_Win(2, :), InitialMap_Win(3, :), 'b');
