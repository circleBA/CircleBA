...
ellipsePath = 'data/td_yaw4/results_p2c.txt';
...
for i = 1:tracking_num
    ...
    [PM_normal_cam{i}, PM_center_cam{i}, Window_normal_cam{i}, Window_center_cam{i}, C_pm{i}, C_win{i}] = perspective_two_circle(ellipses_params{i}, R, intrinsics.K);
    % pause(1);
...
for i = 1:tracking_num-1
    ...
    BAEstimatedPose{i+1} = EstimatedPose{1} * Relative_Trans_opt;
end
...
plot3(PM_Map_ba(1, :), PM_Map_ba(2, :), PM_Map_ba(3, :), 'r', 'MarkerSize', 4); hold on
plot3(Win_Map_ba(1, :), Win_Map_ba(2, :), Win_Map_ba(3, :), 'b', 'MarkerSize', 4);

plot3(InitialMap_PM(1, :), InitialMap_PM(2, :), InitialMap_PM(3, :), 'm', 'MarkerSize', 4);
plot3(InitialMap_Win(1, :), InitialMap_Win(2, :), InitialMap_Win(3, :), 'c', 'MarkerSize', 4);
