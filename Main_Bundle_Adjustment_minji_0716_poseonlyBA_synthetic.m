...
posePath = 'data_astrobee_mixed_data/multiview_mixed_data/groundtruth.txt';
...
%writecell(circle1_points_2D, 'circle1_output.csv');
%writecell(circle2_points_2D, 'circle2_output.csv');
...
% c12d= horzcat(circle2_points_2D{:});
% writematrix(c12d, 'circle1_points.csv');
% c22d= horzcat(circle2_points_2D{:});
% writematrix(c22d, 'circle2_points.csv');


% Visualize
% Compute ellipse parameters for both circles
for i = 1:tracking_num
    ellipse_result1{i}(1, :) = fit_ellipse3(circle1_points_2D{i});
    ellipse_result2{i}(1, :) = fit_ellipse3(circle2_points_2D{i});
end
% for i = 1:tracking_num
...
function ellipse_params = fit_ellipse2(points_2D)
    % Fit an ellipse to 2D projected points 
    % Returns [x_center, y_center, major_axis_length, minor_axis_length, theta]
    ...
    % Get longest points
    distances = vecnorm(centralized_points,2,2);
    
    [Ldistance,Lidx] = maxk(distances,10);
    [Sdistance,Sidx] = mink(distances,10);
       
    major_length = mean(Ldistance) * 2;
    minor_length = mean(Sdistance) * 2;
    
    disp(Lidx);

    major_axis_vector = mean(centralized_points(Lidx,:),1);
    % Compute rotation angle (theta) in the image plane
    % atan2d is used to get the correct quadrant; Y-axis is inverted in image plane
    theta = atan2d(major_axis_vector(2), -major_axis_vector(1)); 

    % Store ellipse parameters
    ellipse_params = [y_center, x_center, major_length, minor_length, 90+theta];
end


function ellipse_params = fit_ellipse3(points_2D)
    ...
    ellipse_params = [Y0_in,X0_in,long_axis,short_axis,rad2deg(orientation_rad) + 90];
end
