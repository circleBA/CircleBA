...
imagePath = 'data/ff_return_journey_forward_optitrack/gray/';
ellipsePath = 'data/ff_return_journey_forward_optitrack/results_p2c/';
posePath = 'data/ff_return_journey_forward_optitrack/groundtruth.txt';
...
% figure;
% k=25;
% plot3(InitialMap_PM_all{1}(1, :), InitialMap_PM_all{1}(2, :), InitialMap_PM_all{1}(3, :), 'bo', 'MarkerSize', 4); hold on;
% plot3(InitialMap_Win_all{1}(1, :), InitialMap_Win_all{1}(2, :), InitialMap_Win_all{1}(3, :), 'ro', 'MarkerSize', 4);
% 
% % % 
% % plot3(InitialMap_PM_all{9}(1, :), InitialMap_PM_all{9}(2, :), InitialMap_PM_all{9}(3, :), 'bo', 'MarkerSize', 2); hold on;
% % plot3(InitialMap_Win_all{9}(1, :), InitialMap_Win_all{9}(2, :), InitialMap_Win_all{9}(3, :), 'ro', 'MarkerSize', 2);
% % 
% % plot3(InitialMap_PM(1, :), InitialMap_PM(2, :), InitialMap_PM(3, :), 'k', 'MarkerSize', 4); hold on;
% % plot3(InitialMap_Win(1, :), InitialMap_Win(2, :), InitialMap_Win(3, :), 'k', 'MarkerSize', 4);
% 
...
for i =1:tracking_num
    ...
    %residual 시각화
    % figure;
    % residuals = EReprojection_motiononly3(opt_params, InitialMap_PM, InitialMap_Win, C_pm{i}, C_win{i}, intrinsics.K);
    % plot(residuals);
    % title(sprintf('Residuals for Frame %d', i));
    % xlabel('Feature Index');
    % ylabel('Residual Value');


    angle_diff2{i} = acos(dot(PM_normal_init, PM_normal_esti)) * 180 / pi;
    if angle_diff2{i} > 100
        ransac2(i) = 0;
    end
    for j = 1:sample_num
...
for i = 1:tracking_num
    k=i;
    plot3(InitialMap_PM_all{k}(1, :), InitialMap_PM_all{k}(2, :), InitialMap_PM_all{k}(3, :), 'bo', 'MarkerSize', 2); hold on;
    plot3(InitialMap_Win_all{k}(1, :), InitialMap_Win_all{k}(2, :), InitialMap_Win_all{k}(3, :), 'ro', 'MarkerSize', 2);

    if i == best_k
        plot_camera_frame(EstimatedPose{best_k}(1:3, 1:3), EstimatedPose{best_k}(1:3, 4), 0.5, 'g'); hold on;     % Relative_Trans_opt = expSE3(opt_params_store{i});  % opt_params를 미리 저장해둔 경우
    ...
    plot_camera_frame(EstimatedPose{9}(1:3, 1:3), EstimatedPose{9}(1:3, 4), 0.5, 'm'); hold on;
    plot_camera_frame(EstimatedPose{18}(1:3, 1:3), EstimatedPose{18}(1:3, 4), 0.5, 'm'); hold on;
    % plot_camera_frame(EstimatedPose{best_k}(1:3, 1:3), EstimatedPose{best_k}(1:3, 4), 0.5, 'g'); hold on;     % Relative_Trans_opt = expSE3(opt_params_store{i});  % opt_params를 미리 저장해둔 경우
